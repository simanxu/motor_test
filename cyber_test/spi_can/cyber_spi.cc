...
void CyberSpi::init_recv_thread() {
  can_recv_thread_ = std::thread(&CyberSpi::RecvMotorStatus, this);
  can_recv_thread_.detach();
}

void CyberSpi::send_loop() {}

int CyberSpi::send_by_bus(unsigned char ch, unsigned short id, unsigned char* data, unsigned char len) {
  int ii;
...
int CyberSpi::send_by_bus(unsigned char ch, unsigned short id) {
  struct can_frame frame;
  memset(&frame, 0, sizeof(struct can_frame));
  frame.can_id = id;
  ...
  switch (ch) {
    case 1:
      ...
      break;
    default:
      nbytes = write(can_s0, &tx_msg, sizeof(tx_msg));
      break;
  }
...
int CyberSpi::send_by_bus_block(unsigned char ch, unsigned short id, unsigned char* data, unsigned char len) {
  int ii;
  struct can_frame frame;
  memset(&frame, 0, sizeof(struct can_frame));
  frame.can_id = id;
...
int CyberSpi::send_by_bus_nonblock(unsigned char ch, unsigned short id, unsigned char* data, unsigned char len) {
  int ii;
  struct can_frame frame;
  memset(&frame, 0, sizeof(struct can_frame));
  frame.can_id = id;
...
int CyberSpi::send_by_bus0_timeout(unsigned short id, unsigned char* data, unsigned char len) {
  int ii;
  int rsv_timeout = kCanRecvTimeout;
  struct can_frame frame;
  memset(&frame, 0, sizeof(struct can_frame));
...
int CyberSpi::send_by_bus1_timeout(unsigned short id, unsigned char* data, unsigned char len) {
  int ii;
  struct can_frame frame;
  int rsv_timeout = kCanRecvTimeout;
  memset(&frame, 0, sizeof(struct can_frame));
...
void CyberSpi::data_proc(unsigned char ch, struct can_frame* frame) {
...
void CyberSpi::set_en(int idx, bool en) {
...
void CyberSpi::get_var(int idx, int mode) {
  unsigned short id = motors[idx].id;
  unsigned short cmd;
  switch (mode) {
    case 0:
...
void CyberSpi::set_value(int idx, int mode, float value) {
...
void CyberSpi::motor_cmd(int idx, int mode, int can_cmd, float value) {
  unsigned char tx_buff[8] = {0};
  unsigned short id = motors[idx].id;
...
void CyberSpi::float_char(float value, unsigned char* buff) {
