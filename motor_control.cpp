...
int Motor_Control::send_by_bus(unsigned char ch, unsigned short id, unsigned char* data, unsigned char len) {
  int ii;
...
int Motor_Control::send_by_bus_block(unsigned char ch, unsigned short id, unsigned char* data, unsigned char len) {
  int ii;
  struct can_frame frame;
  memset(&frame, 0, sizeof(struct can_frame));
  frame.can_id = id;
...
int Motor_Control::send_by_bus0_timeout(unsigned short id, unsigned char* data, unsigned char len) {
  int ii;
...
int Motor_Control::send_by_bus1_timeout(unsigned short id, unsigned char* data, unsigned char len) {
  int ii;
  struct can_frame frame;
  int rsv_timeout = CAN_TIMEOUT;
  memset(&frame, 0, sizeof(struct can_frame));
  ...
  if (id > 0 && id < 13) {
    ...
    switch (cmd) {
      case 0x0C:  // position value
        ...
        motors[idx].position_real = tmpfloat * motors[idx].position_k - motors[idx].position_zero;
        break;
